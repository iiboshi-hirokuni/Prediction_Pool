...
%% Forecast Errors (10)
Eta_t = [eta_c; eta_lambda; eta_w; eta_q; eta_r_k; eta_pi; eta_i; ...
     eta_z_b; eta_z_psi; eta_z_star];

%% Exogenous Shock Variables (10)
Epsilon_t = [epsilon_b; epsilon_g; epsilon_w; epsilon_p; epsilon_nu; ...
     epsilon_r; epsilon_z; epsilon_psi; epsilon_efp; epsilon_nw];
...
Eq_S5 = -Et_r_E_t1 + (1-(1-delta)/r_E/psi)*Et_r_k_t1+...
        ((1-delta)/r_E/psi)*Et_q_t1 - q_t - Et_z_psi_t1;

Eq_S6 = -Et_r_E_t1 + r_n_t - Et_pi_t1 - mu_E*(n_t-q_t-k_t) + z_efp_t;
...
System_of_Eq = [Eq_S1; Eq_S2; Eq_S3; Eq_S4; Eq_S5; ...
  ...
  Eq_P6; Eq_P7; Eq_P8; Eq_P9; Eq_P10; ...
  Eq_I1; Eq_I2; Eq_I3];
...
ystr = {'Yt', 'Ct', 'It', 'Wt', 'Lt', 'Pt', 'Pi/Pt',...
  ...
  yyirf_total  = zeros(nirf,nvar,nshock);  % 2014/08/10 追加
...
%     for j = 1:9
%        subplot(3, 3, j)                                 % 2014/08/10 修正　　　　　　　　　　　　　　　　　　　　
%        plot(1:nirf, yyirf(:,j),'b')
%        title(strcat(titlestr(sh_ind),ystr(j)))
%     end
    
    yyirf_total(:,:,sh_ind)  = yyirf;  % 2014/08/10 追加
