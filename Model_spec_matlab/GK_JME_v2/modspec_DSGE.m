...
syms e_a e_ksi e_g e_i Keff_lg w_lg VMPK_lg Welf_lg Welf_ld Welf_lg Welf_lda
...
Y_ta = [Y_lg; Ym_lg; K_lg; L_lg; I_lg; C_lg; G_lg; Q_lg;...
        ...
        g_lg; infl_lg; inflstar_lg; ...
        C_lda; Lambda_lda; In_lda; Rk_lda; infl_lda; F_lda; Z_lda];
        %Keff_lg; w_lg; VMPK_lg; Welf_lg; Welf_lda];
...
Eq23 = -exp(i) + exp(i_lg)^rho_i*((1/betta)*exp(infl)^kappa_pi*(exp(X)/(epsilon/(epsilon-1)))^(kappa_y))^(1-rho_i)*exp(e_i);
...
%EqI1 = -exp(Keff) + exp(ksi)*exp(K_lg);
%
%%%27. Wages
%EqI2 = -exp(w) + exp(Pm)*(1-alfa)*exp(Y)/exp(L);
%
%%%28. Marginal value product of capital
%EqI3 = -exp(VMPK) + exp(Pm)*alfa*exp(Y)/(exp(ksi)*exp(K_lg));
%
%%%29. Welfare
%EqI4 = -Welf + log(exp(C)-hh*exp(C_lg))-chi*exp(L)^(1+varphi)/(1+varphi)+betta*Welf_ld;
%
%EqI5 = -Welf + Welf_lda;
...
sig_chol = diag([0.01,0.05,0.01,0.01])*100; 

titlestr = {'eps a :'; 'eps xi :'; 'eps g :'; 'eps i :'};

ystr = {'xi'; 'R'; 'Y'; 'C'; 'I'; 'K'; 'L'; 'Q'; 'infl'; 'i'};
index1 = [2,1];
nvar = size(Z0, 1);
index2 = [1:2,4:9,11:12];
