...
% cd H:\work\Hprograming\matlab\GK_JME
...
syms C_ld Lambda_ld In_ld Rk_ld R_ld infl_ld F_ld Z_ld ...
     eta_C eta_Lambda eta_In eta_Rk eta_R eta_infl ...
     C_lda Lambda_lda In_lda Rk_lda R_lda infl_lda F_lda Z_lda 
...
Y_t = [Y; Ym; K; L; I; C; G; Q; varrho; Lambda; Rk; R; Pm; U; D; X; F; ...
       ...
       C_ld; Lambda_ld; In_ld; Rk_ld; R_ld; infl_ld; F_ld; Z_ld];
...
% Forecast Errors
Eta_t = [eta_C; eta_Lambda; eta_Rk; eta_R; eta_infl];
...
%4. Arbitrage
Eq4 = -betta*Lambda_ld*Rk_ld + betta*Lambda_ld*R_ld;
...
Eq8 = -Q + 1+eta_i/2*((In+I_ss)/(In_lg+I_ss)-1)^2+eta_i*((In+I_ss)/(In_lg+I_ss)-1)...
    *(In+I_ss)/(In_lg+I_ss)-betta*Lambda_ld*eta_i*((In_ld+I_ss)/(In+I_ss)-1)...
    *((In_ld+I_ss)/(In+I_ss))^2;
...
Eq14 = -Y + C+G+I+eta_i/2*((In+I_ss)/(In_lg+I_ss)-1)^2*(In+I_ss);

%15. Wholesale, retail output
Eq15 = -Ym + Y*D;

%16. Price dispersion
Eq16 = -D + gam*D_lg*infl_lg^(-gam_P*epsilon)*infl^epsilon+(1-gam)*((1-gam*infl_lg^(gam_P*(1-gam))*infl^(gam-1))/(1-gam))^(-epsilon/(1-gam));

%17. Markup
Eq17 = -X + 1/Pm;

%18. Optimal price choice
Eq18 = -F + Y*Pm+betta*gam*Lambda_ld*infl_ld^epsilon*(infl)^(-epsilon*gam_P)*F_ld;

%19.
Eq19 = -Z + Y+betta*gam*Lambda_ld*infl_ld^(epsilon-1)*infl^(gam_P*(1-epsilon))*Z_ld;

%20. Optimal price choice
Eq20 = -inflstar + epsilon/(epsilon-1)*F/Z*infl;

%21. Price index
Eq21 = -infl^(1-epsilon) + gam*infl_lg^(gam_P*(1-epsilon))+(1-gam)*(inflstar)^(1-epsilon);

%22. Fisher equation
Eq22 = -i + R + infl_ld;

%23. Interest rate rule
Eq23 = -i + rho_i*i + (1-rho_i)*(1/betta + kappa_pi*infl + ...
...
System_of_Eq = [Eq1; Eq2; Eq3; Eq4; Eq5;...
             ...
             EqF1; EqF2; EqF3; EqF4; EqF5; EqF6; EqF7 ;EqF8];
